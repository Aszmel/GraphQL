...
const Mutations = {
  async createItem(parent, args, ctx, info) {
    ...
    const item = await ctx.db.mutation.createItem(
      {
        data: {
          //this is how in prisma bind relation between Item & User
          user: {
            connect: {
  ...
  async updatePermissions(parent, args, ctx, info) {
    ...
    //check if they have permission to do that
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    //check if the item is already in the cart (so add or increment)
    if (existingCartItem) {
    ...
    //if item not in cart, add fresh item to cart for that user
